...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const differentTime = document.getElementById("different-time");
  ...
  function showScreen(screenId) {
    const targetId = protectedScreens.has(screenId) && !isLoggedIn()
      ? "screen-login"
      : screenId;
  ...
  function validateDifferentAnswer(index) {
    if (!differentGame || differentGame.finished || differentGame.roundLocked) return;
  ...
  function updateDifferentTimer() {
    ...
    differentTime.textContent = `${seconds}s`;
  }
  ...
  function finishDifferentGame() {
    ...
    differentHits.textContent = differentGame.hits;
    differentErrors.textContent = differentGame.errors;
    differentResultText.textContent = `${message} Tiempo usado: ${seconds}s.`;
  ...
  differentFamilySelect.addEventListener("change", startDifferentGame);
  differentDifficultySelect.addEventListener("change", startDifferentGame);
  ...
  newDifferentBtn.addEventListener("click", startDifferentGame);
  saveDifferentBtn.addEventListener("click", () => {
    saveDifferentResult();
    saveDifferentBtn.textContent = "Resultado guardado";
  });
  playAgainDifferentBtn.addEventListener("click", startDifferentGame);
